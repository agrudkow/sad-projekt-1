...
```{r}
count_exist_sec_per_min <- function(start_time) {
...
df_count_per_minute <- count_exist_sec_per_min(min_tf_result$min_tf[2])
df_count_per_minute
```

```{r}
df_count_per_minute <- prepare_df_count_per_minute(min_tf_result$min_tf[2])
check_time <- c()
check_num_secs <- c()
for(i in 1:120) {
  time <- df_count_per_minute[i, ]['times']
  pattern <- paste("^", time, sep = "")
  num_secs <- filter(min_tf_result$min_df, str_detect(time, pattern)) %>% nrow
  df_count_per_minute$count[df_count_per_minute$times==time] <- num_secs
  check_time <- c(check_time, time)
  check_num_secs <- c(check_num_secs, num_secs)
}
...
num_secs <- filter(min_tf_result$min_df, str_detect(time, pattern)) %>% nrow
df_count_per_minute$count[df_count_per_minute$times==time] <- num_secs
df_count_per_minute$count[df_count_per_minute$times=="1132"] <- num_secs
df_count_per_minute$count[df_count_per_minute$times=="1135"] <- num_secs
