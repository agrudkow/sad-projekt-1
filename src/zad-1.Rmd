...
```{r}
volumes_sum_by_hour <- transactions$continous %>% sum_volumes_by_hour
...
### Zad. 2. Wyznacz dzień i jego 2-godzinny przedział czasu rozpoczynający się o pełnej godzinie między 10:00 a 14:00 włącznie, w którym jest najmniej sekund, w których przeprowadzane były transakcje na akcjach danej spółki.
...
### Zad. 2 (i). W każdej kolejnej minucie rozważanego przedziału wyznacz liczbę sekund, w których przeprowadzane były transakcje (otrzyma się w ten sposób 120 liczb z przedziału [0, 60]),
```{r}
...
### Zad. 2 (ii). zamodeluj ww. liczby za pomocą rozkładu Poissona
```{r}
...
```
### Zad. 2c (iii). porównaj rozkład modelowy z danymi.
