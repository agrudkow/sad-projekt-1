...
draw_plots <- function(data) {
  ...
  boxplot(data, xlab = "Zmiana procentowa", horizontal = TRUE)
...
Rozkład wolumenia transakcji fazy otwarcia (Wykres pudełkowy)
```{r}
ggplot(opening, aes(y=volume)) + geom_boxplot()
```

Rozkład wolumenia transakcji fazy notowania ciagłego (Wykres pudełkowy)
```{r}
ggplot(continous, aes(y=volume)) + geom_boxplot()
```

Rozkład wolumenia transakcji fazy zamknięcia (Wykres pudełkowy)
```{r}
...
min_secs_data_df <- load_data("./data/p1csv_new/PGE.csv") %>% prepare_data %>% filter(time >= min_secs_start_time, time <= min_secs_finish_time, date==min_secs_date) %>% distinct(time, .keep_all = TRUE)
```
